...
class NlAccNkSxH(models.Model):
    ...
    def action_report_phieu_nhap(self):
        return {
            ...
            'url': f'/download/phieu_nhap_sx/{self.id}',
            'target': 'self',
        }
    ...
    def write(self, vals):
        ...
        for record in self:
            ...
            vals_dict = {
                ...
                "MENU_ID": self._get_parent_value(record, vals, 'MENU_ID').id or 381,
        ...
        for record in self:
            ...
            self.env['nl.acc.tong.hop'].sudo().search([('ACC_NK_SX_D', 'in', all_d_records.ids)]).unlink()
