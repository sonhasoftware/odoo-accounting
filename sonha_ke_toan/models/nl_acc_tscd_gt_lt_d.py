...
class AccTscdGtLtD(models.Model):
    _name = 'nl.acc.tscd.gt.lt.d'

    ACC_AP_H = fields.Many2one('nl.acc.tscd.gt.lt.h', string="ID Header", store=True)
    ...
    def _get_don_gia(self):
        for r in self:
            check = self.env['sonha.phan.quyen.nl'].sudo().search([('MENU', '=', 383),
                                                                   ('GIA_MUA', '=', True)])
    ...
    def create(self, vals):
        ...
        if vals.get('ACC_AP_H'):
            related = self.env['nl.acc.tscd.gt.lt.h'].sudo().browse(vals['ACC_AP_H'])
            if related.exists():
                vals.update({
        ...
        rec = super(AccTscdGtLtD, self).create(vals)
        ...
        for data in rows:
            ...
            new_d = super(AccTscdGtLtD, self).create(mapped_vals)
        ...
        self._cr.commit()
        # self.env['nl.acc.tong.hop'].sudo().search([('ACC_GT_LT', '=', None)]).unlink()
