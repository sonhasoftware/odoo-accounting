...
class NlAccPcD(models.Model):
    _name = 'nl.acc.pc.d'

    ACC_AP_H = fields.Many2one('nl.acc.pc.h', string="ID Header", store=True)
    ...
    def _get_don_gia(self):
        for r in self:
            check = self.env['sonha.phan.quyen.nl'].sudo().search([('MENU', '=', 393),
                                                                   ('GIA_MUA', '=', True)])
    ...
    def create(self, vals):
        ...
        if vals.get('ACC_AP_H'):
            related = self.env['nl.acc.pc.h'].sudo().browse(vals['ACC_AP_H'])
            if related.exists():
                vals.update({
        ...
        rec = super(NlAccPcD, self).create(vals)
        ...
        #     new_d = super(NlAccPcD, self).create(mapped_vals)
