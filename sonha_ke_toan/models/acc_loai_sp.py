...
class AccLoaiSP(models.Model):
    ...
    LOAI_SP = fields.Integer(string="Loại SP", store=True, readonly=True)
    ...
    def _search(self, args, offset=0, limit=None, order=None, access_rights_uid=None):
        ...
        query = "SELECT * FROM public.fn_acc_loai_sp(%s, %s)"
        ...
        return super(AccLoaiSP, self)._search(
            new_domain,
            offset=offset,
            limit=limit,
            order=order,
    ...
    def create(self, vals):
        ...
        rec = super(AccLoaiSP, self).create(vals)
        rec.LOAI_SP = rec.id
        dvcs = rec.DVCS.id
        self.env.cr.execute("CALL public.update_cap(%s, %s);", ['acc_loai_sp', dvcs])

        return rec

    def write(self, vals):
        res = super(AccLoaiSP, self).write(vals)
    ...
    def unlink(self):
        res = super(AccLoaiSP, self).unlink()
        dvcs = self.DVCS.id
        self.env.cr.execute("CALL public.update_cap(%s, %s);", ['acc_loai_sp', dvcs])
        return res
