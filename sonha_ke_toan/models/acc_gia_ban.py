...
class AccGiaBan(models.Model):
    _name = 'acc.gia.ban'
    _rec_name = 'HANG_HOA_NAME'

    NGAY_HL = fields.Date(string="Ngày hiệu lực", store=True)
    ...
    KHACH_HANG = fields.Many2one('acc.khach.hang', string="Mã KH", store=True)
    KHACH_HANG_NAME = fields.Char(related='KHACH_HANG.TEN', string="Tên hàng", store=True)

    LOAI_KH = fields.Many2one('acc.loai.kh', string="Loại KH", store=True)
    DON_GIA = fields.Float(string="Đơn giá", store=True)
    ...
    def _search(self, args, offset=0, limit=None, order=None, access_rights_uid=None):
        ...
        query = "SELECT * FROM public.fn_acc_gia_ban(%s, %s)"
        ...
        return super(AccGiaBan, self)._search(
            new_domain,
            offset=offset,
            limit=limit,
            order=order,
