...
class NLAccApXhtlD(models.Model):
    _name = 'nl.acc.ap.xhtl.d'

    ACC_AP_H = fields.Many2one('nl.acc.ap.xhtl.h', string="ID Header", store=True)
    ...
    def create(self, vals):
        ...
        if vals.get('ACC_AP_H'):
            related = self.env['nl.acc.ap.xhtl.h'].sudo().browse(vals['ACC_AP_H'])
            if related.exists():
                vals.update({
        ...
        rec = super(NLAccApXhtlD, self).create(vals)
        ...
        for data in rows:
            ...
            new_d = super(NLAccApXhtlD, self).create(mapped_vals)
        ...
        self._cr.commit()
        # self.env['nl.acc.tong.hop'].sudo().search([('ACC_XHTL_D', '=', None)]).unlink()
