...
class NLAccApCkD(models.Model):
    ...
    def _get_don_gia(self):
        for r in self:
            check = self.env['sonha.phan.quyen.nl'].sudo().search([('MENU', '=', 386),
                                                                   ('GIA_MUA', '=', True)])
    ...
    def create(self, vals):
        ...
        if vals.get('ACC_AP_H'):
            ...
            if related.exists():
                vals.update({
                    ...
                    'MENU_ID': related.MENU_ID.id if related.MENU_ID else 386,
        ...
        rec = super(NLAccApCkD, self).create(vals)
        ...
        for data in rows:
            ...
            new_d = super(NLAccApCkD, self).create(mapped_vals)
        ...
        for r in records_to_sync:
            ...
            self._cr.commit()
            #self.env['nl.acc.tong.hop'].sudo().search([('ACC_CK_D', '=', None)]).unlink()
