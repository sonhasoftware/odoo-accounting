...
class NlAccHdbhH(models.Model):
    ...
    GHI_CHU = fields.Char(string="Ghi chú", store=True, default="Hóa đơn bán hàng", size=200)
    ...
    def write(self, vals):
            ...
            if 'ACC_AP_D' in vals or 'ACC_SP_D' in vals:
                ...
                for cmd in new_d_records:
                    if cmd[0] == 0:  # Create command
                        ...
                        d_record = self.env['nl.acc.hdbh.d'].browse(cmd[1])
                ...
                all_d_records = self.env['nl.acc.hdbh.d'].search([('ACC_AP_H', '=', record.id)])
                d_records_to_validate = []
        ...
        res = super(NlAccHdbhH, self).write(vals)
        ...
        for record in self:
            ...
            self.env['nl.acc.tong.hop'].sudo().search([('ACC_HDBH', 'in', all_d_records.ids)]).unlink()
            self.env['nl.acc.hdbh.d'].sudo().search([('id', 'in', all_d_records.ids)]).unlink()

            if d_vals_list:
                self.env['nl.acc.hdbh.d'].sudo().create(d_vals_list)
