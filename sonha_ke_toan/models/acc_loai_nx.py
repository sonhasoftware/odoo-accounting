...
class AccLoaiNX(models.Model):
    ...
    LOAI_NX = fields.Integer(string="Loại phân xưởng", store=True, readonly=True)
    ...
    def _search(self, args, offset=0, limit=None, order=None, access_rights_uid=None):
        ...
        query = "SELECT * FROM public.fn_acc_loai_nx(%s, %s)"
        ...
        return super(AccLoaiNX, self)._search(
            new_domain,
            offset=offset,
            limit=limit,
            order=order,
    ...
    def create(self, vals):
        ...
        rec = super(AccLoaiNX, self).create(vals)
        rec.LOAI_NX = rec.id
        dvcs = rec.DVCS.id
        self.env.cr.execute("CALL public.update_cap(%s, %s);", ['acc_loai_nx', dvcs])

        return rec

    def write(self, vals):
        res = super(AccLoaiNX, self).write(vals)
    ...
    def unlink(self):
        res = super(AccLoaiNX, self).unlink()
        dvcs = self.DVCS.id
        self.env.cr.execute("CALL public.update_cap(%s, %s);", ['acc_loai_nx', dvcs])
        return res
