...
class AccHangHoa(models.Model):
    ...
    TK_KHO_ID = fields.Many2one('acc.tai.khoan', string="TK kho", store=True)
    TK_KHO = fields.Char(related='TK_KHO_ID.MA', string="TK kho", store=True)

    TK_GV_ID = fields.Many2one('acc.tai.khoan', string="TK GV", store=True)
    TK_GV = fields.Char(related='TK_GV_ID.MA', string="TK GV", store=True)

    TK_DT_ID = fields.Many2one('acc.tai.khoan', string="TK DT", store=True)
    TK_DT = fields.Char(related='TK_DT_ID.MA', string="TK DT", store=True)

    LOAI_HH = fields.Many2one('acc.loai.hh', string="Ngành hàng", store=True)
    CHINH_PHU = fields.Selection([('chinh', "Chính"),
                                  ('phu', "Phụ")],
    ...
    #     query = "SELECT * FROM public.fn_acc_hang_hoa(%s, %s)"
    ...
    #
    #     return super(AccHangHoa, self)._search(
    #         new_domain,
    #         offset=offset,
    #         limit=limit,
    #         order=order,
    ...
    def unlink(self):
        res = super(AccHangHoa, self).unlink()
        dvcs = self.DVCS.id
        self.env.cr.execute("CALL public.update_cap(%s, %s);", ['acc_hang_hoa', dvcs])
        return res
