...
class NlAccHdbhD(models.Model):
    _name = 'nl.acc.hdbh.d'

    ACC_AP_H = fields.Many2one('nl.acc.hdbh.h', string="ID Header", store=True)
    ...
    def _get_don_gia(self):
        for r in self:
            check = self.env['sonha.phan.quyen.nl'].sudo().search([('MENU', '=', 403),
                                                                   ('GIA_MUA', '=', True)])
    ...
    def create(self, vals):
        ...
        if vals.get('ACC_AP_H'):
            related = self.env['nl.acc.hdbh.h'].sudo().browse(vals['ACC_AP_H'])
            if related.exists():
                vals.update({
                    ...
                    'MENU_ID': related.MENU_ID.id if related.MENU_ID else 400,
        ...
        rec = super(NlAccHdbhD, self).create(vals)
        ...
        for data in rows:
            ...
            new_d = super(NlAccHdbhD, self).create(mapped_vals)
        ...
        self._cr.commit()
        # self.env['nl.acc.tong.hop'].sudo().search([('ACC_HDBH', '=', None)]).unlink()
