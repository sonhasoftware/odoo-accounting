...
class AccTscdGgLtD(models.Model):
    _name = 'nl.acc.tscd.gg.lt.d'

    ACC_AP_H = fields.Many2one('nl.acc.tscd.gg.lt.h', string="ID Header", store=True)
    ...
    def _get_don_gia(self):
        for r in self:
            check = self.env['sonha.phan.quyen.nl'].sudo().search([('MENU', '=', 385),
                                                                   ('GIA_MUA', '=', True)])
    ...
    def create(self, vals):
        ...
        if vals.get('ACC_AP_H'):
            related = self.env['nl.acc.tscd.gg.lt.h'].sudo().browse(vals['ACC_AP_H'])
            if related.exists():
                vals.update({
                    ...
                    'MENU_ID': related.MENU_ID.id if related.MENU_ID else 385,
        ...
        rec = super(AccTscdGgLtD, self).create(vals)
        ...
        for data in rows:
            ...
            new_d = super(AccTscdGgLtD, self).create(mapped_vals)
        ...
        for r in records_to_sync:
            ...
            self._cr.commit()
            # self.env['nl.acc.tong.hop'].sudo().search([('ACC_GG_LT', '=', None)]).unlink()
