...
class AccTscdGgNcH(models.Model):
    _name = 'nl.acc.tscd.gg.nc.h'
    _order = 'NGAY_CT DESC'
    _rec_name = 'CHUNG_TU'
    ...
    GHI_CHU = fields.Char(string="Ghi chú", store=True, default="Ghi giảm TCSĐ (Nguyên chiếc)", size=200)
    ...
    ACC_SP_D = fields.One2many(
        comodel_name="nl.acc.tscd.gg.nc.d",
    ...
    def default_get(self, fields_list):
        res = super(AccTscdGgNcH, self).default_get(fields_list)
        ...
        res.update({
            ...
            'MENU_ID': permission.MENU.id or 384,
    ...
    def create(self, vals):
        ...
        for recs in temp_rec.ACC_SP_D:
            ...
            vals_dict = {
                ...
                "MENU_ID": temp_rec.MENU_ID.id or 384,
        ...
        rec = super(AccTscdGgNcH, self).create(vals)
        query = "SELECT * FROM fn_chung_tu_tu_dong(%s, %s)"
        self.env.cr.execute(query, ('menu_384', str(rec.NGAY_CT)))
        rows = self.env.cr.fetchall()
        if rows:
    ...
    def write(self, vals):
            ...
            if 'ACC_AP_D' in vals or 'ACC_SP_D' in vals:
                ...
                for cmd in new_d_records:
                    if cmd[0] == 0:  # Create command
                        ...
                        d_record = self.env['nl.acc.tscd.gg.nc.d'].browse(cmd[1])
                ...
                all_d_records = self.env['nl.acc.tscd.gg.nc.d'].search([('ACC_AP_H', '=', record.id)])
                d_records_to_validate = []
        ...
        res = super(AccTscdGgNcH, self).write(vals)
        ...
        for record in self:
            ...
            self.env['nl.acc.tong.hop'].sudo().search([('ACC_GG_NC', 'in', all_d_records.ids)]).unlink()
            self.env['nl.acc.tscd.gg.nc.d'].sudo().search([('id', 'in', all_d_records.ids)]).unlink()

            if d_vals_list:
                self.env['nl.acc.tscd.gg.nc.d'].sudo().create(d_vals_list)
