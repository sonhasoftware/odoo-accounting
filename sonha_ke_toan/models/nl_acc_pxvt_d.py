...
class NlAccApPxvtD(models.Model):
    _name = 'nl.acc.ap.pxvt.d'

    ACC_AP_H = fields.Many2one('nl.acc.ap.pxvt.h', string="ID Header", store=True)
    ...
    SO_LUONG2 = fields.Float("SL2", store=True)
    ...
    PXUONG = fields.Many2one('acc.loai.nx', string="Phân xưởng", store=True)
    TSCD = fields.Many2one('acc.tscd', string="TSCĐ", store=True)
    #
        ...
        for r in self:
            if permission.GIA_MUA:
                don_gia = self.env['acc.gia.mua'].sudo().search([('HANG_HOA', '=', r.HANG_HOA.id),
                                                                 ('NGAY_HL', '<=', r.NGAY_CT)], order='NGAY_HL desc',
                                                                limit=1)
    ...
    def _get_don_gia(self):
        for r in self:
            check = self.env['sonha.phan.quyen.nl'].sudo().search([('MENU', '=', 402),
                                                                   ('GIA_MUA', '=', True)])
            if r.ACC_AP_H.DG_THEO_TIEN:
                ...
                self.env.cr.execute(query,
                                    (r.DVCS.id, r.HANG_HOA.id, r.ACC_AP_H.KHACH_HANG.id, str(r.ACC_AP_H.NGAY_CT)))
    ...
    def create(self, vals):
        ...
        if vals.get('ACC_AP_H'):
            related = self.env['nl.acc.ap.pxvt.h'].sudo().browse(vals['ACC_AP_H'])
            if related.exists():
                vals.update({
                    ...
                    'MENU_ID': related.MENU_ID.id if related.MENU_ID else 405,
        ...
        rec = super(NlAccApPxvtD, self).create(vals)
        ...
        for data in rows:
            ...
            new_d = super(NlAccApPxvtD, self).create(mapped_vals)
        ...
        self._cr.commit()
        # self.env['nl.acc.tong.hop'].sudo().search([('ACC_PXVT_D', '=', None)]).unlink()
