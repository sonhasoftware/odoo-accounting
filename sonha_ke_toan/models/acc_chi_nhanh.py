...
class AccChiNhanh(models.Model):
    _name = 'acc.chi.nhanh'
    _order = 'MA,CAP,DVCS'
    _rec_name = 'MA_TEN'
    ...
    CHI_NHANH = fields.Integer(string="Chi Nhánh", store=True, readonly=True)
    ...
    def _search(self, args, offset=0, limit=None, order=None, access_rights_uid=None):
        ...
        query = "SELECT * FROM public.fn_acc_chi_nhanh(%s, %s)"
        ...
        return super(AccChiNhanh, self)._search(
            new_domain,
            offset=offset,
            limit=limit,
            order=order,
    ...
    def create(self, vals):
        ...
        rec = super(AccChiNhanh, self).create(vals)
        rec.CHI_NHANH = rec.id
        dvcs = rec.DVCS.id
        self.env.cr.execute("CALL public.update_cap(%s, %s);", ['acc_chi_nhanh', dvcs])

        return rec

    def write(self, vals):
        res = super(AccChiNhanh, self).write(vals)
    ...
    def unlink(self):
        res = super(AccChiNhanh, self).unlink()
        dvcs = self.DVCS.id
        self.env.cr.execute("CALL public.update_cap(%s, %s);", ['acc_chi_nhanh', dvcs])
        return res
