...
class NLAccHddvD(models.Model):
    _name = 'nl.acc.hddv.d'

    ACC_AP_H = fields.Many2one('nl.acc.hddv.h', string="ID Header", store=True)
    ...
    def _get_don_gia(self):
        for r in self:
            check = self.env['sonha.phan.quyen.nl'].sudo().search([('MENU', '=', 401),
                                                                   ('GIA_MUA', '=', True)])
    ...
    def create(self, vals):
        ...
        if vals.get('ACC_AP_H'):
            related = self.env['nl.acc.hddv.h'].sudo().browse(vals['ACC_AP_H'])
            if related.exists():
                vals.update({
                    ...
                    'MENU_ID': related.MENU_ID.id if related.MENU_ID else 399,
        ...
        rec = super(NLAccHddvD, self).create(vals)
        ...
        for data in rows:
            ...
            query = f"""
                INSERT INTO nl_acc_hddv_d ({", ".join(columns)})
        ...
        for r in records_to_sync:
            ...
            self._cr.commit()
            # self.env['nl.acc.tong.hop'].sudo().search([('ACC_HDDV', '=', None)]).unlink()
