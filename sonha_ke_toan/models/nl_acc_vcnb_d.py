...
class NlAccVcnbD(models.Model):
    _name = 'nl.acc.vcnb.d'

    ACC_AP_H = fields.Many2one('nl.acc.vcnb.h', string="ID Header", store=True)
    ...
    def create(self, vals):
        ...
        if vals.get('ACC_AP_H'):
            related = self.env['nl.acc.vcnb.h'].sudo().browse(vals['ACC_AP_H'])
            if related.exists():
                vals.update({
                    ...
                    'MENU_ID': related.MENU_ID.id if related.MENU_ID else 401,
        ...
        rec = super(NlAccVcnbD, self).create(vals)
        ...
        for data in rows:
            ...
            new_d = super(NlAccVcnbD, self).create(mapped_vals)
        ...
        for r in records_to_sync:
            ...
            self._cr.commit()
            # self.env['nl.acc.tong.hop'].sudo().search([('ACC_VCNB', '=', None)]).unlink()
