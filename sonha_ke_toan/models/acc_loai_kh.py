...
class AccLoaiKH(models.Model):
    ...
    LOAI_KH = fields.Integer(string="Loại KH", store=True, readonly=True)
    ...
    def _search(self, args, offset=0, limit=None, order=None, access_rights_uid=None):
        ...
        query = "SELECT * FROM public.fn_acc_loai_kh(%s, %s)"
        ...
        return super(AccLoaiKH, self)._search(
            new_domain,
            offset=offset,
            limit=limit,
            order=order,
    ...
    def create(self, vals):
        ...
        rec = super(AccLoaiKH, self).create(vals)
        rec.LOAI_KH = rec.id
        dvcs = rec.DVCS.id
        self.env.cr.execute("CALL public.update_cap(%s, %s);", ['acc_loai_kh', dvcs])

        return rec

    def write(self, vals):
        res = super(AccLoaiKH, self).write(vals)
    ...
    def unlink(self):
        res = super(AccLoaiKH, self).unlink()
        dvcs = self.DVCS.id
        self.env.cr.execute("CALL public.update_cap(%s, %s);", ['acc_loai_kh', dvcs])
        return res
